...
const rate = (b, a) => b ? Math.round(a / b * 1e3) / 10 + "%" : a ? "超支" : "—";
...
async function exportMonthXlsx(mlMonth2) {
  ...
  try {
    ...
    const s1 = [];
    s1.push([`${year} 年度预算执行明细账（${mLabel}）`]);
    s1.push([`编制单位：${org}`, "", "", "", "", "", `单位：人民币元`]);
    s1.push([`编制日期：${today}`, "", "", "", "", "", `编制人：${S.member.name}`]);
    s1.push([]);
    s1.push(["序号", "预算科目", "明细说明", "预算金额", "决算金额（实际）", "差异（决算-预算）", "执行率", "支付/报销时间", "提交人", "审批人", "状态", "备注"]);
    let tb = 0, ta = 0;
    rows.forEach((r, i) => {
      const b = r.budget_amount == null ? null : +r.budget_amount;
      const counted2 = r.status === "approved" || r.status === "paid";
      const a = counted2 && r.amount != null ? +r.amount : r.status === "pending" && r.amount != null ? +r.amount : null;
      if (b) tb += b;
      if (counted2 && r.amount != null) ta += +r.amount;
      s1.push([
        i + 1,
        r.cat_name,
        r.title,
        n2(b) ?? "/",
        r.status === "rejected" ? "—" : n2(a) ?? "",
        b != null && counted2 && a != null ? n2(a - b) : "",
        b != null && counted2 && a != null ? rate(b, a) : "",
        r.pay_date || "",
    ...
    s1.push([]);
    s1.push(["", "合计", "", n2(tb), n2(ta), n2(ta - tb), rate(tb, ta), "", "", "", "", "决算=审批通过/已打款合计"]);
    s1.push([]);
    s1.push(["复核（审批人）：", "", "", "", "负责人：", "", "", "", "会计基础规范：收付实现制口径，以审批通过作为决算确认时点"]);
    const ws1 = X.utils.aoa_to_sheet(s1);
    ws1["!cols"] = [{ wch: 5 }, { wch: 26 }, { wch: 42 }, { wch: 12 }, { wch: 14 }, { wch: 14 }, { wch: 9 }, { wch: 16 }, { wch: 14 }, { wch: 10 }, { wch: 16 }, { wch: 30 }];
    ws1["!merges"] = [{ s: { r: 0, c: 0 }, e: { r: 0, c: 11 } }];
    const s2 = [];
    s2.push([`${S.meta.yearLabel || year + " 年度"} 预算执行汇总表`]);
    s2.push([`编制单位：${org}`, "", "", "", `单位：人民币元`]);
    s2.push([`编制日期：${today}`, "", "", "", `编制人：${S.member.name}`]);
    s2.push([]);
    s2.push(["预算科目（大项）", "年度预算", "累计实际（含审批通过提报）", "差异（实际-预算）", "执行率", "备注"]);
    S.cats.forEach((c) => {
      const cb = catB(c), ca = catA(c);
      s2.push([
        c.name + (c.unplanned ? "（计划外）" : ""),
        n2(cb),
        n2(ca),
        n2(ca - cb),
        c.unplanned ? "不占预算额度" : rate(cb, ca),
        ""
      ]);
      c.items.forEach((it) => {
        const ib = (it.b || []).reduce((x, y) => x + (+y || 0), 0);
        const ia = (it.a || []).reduce((x, y) => x + (+y || 0), 0) + S.records.filter((r) => (r.status === "approved" || r.status === "paid") && r.item_id === it.id).reduce((x, r) => x + (+r.amount || 0), 0);
        if (!ib && !ia) return;
        s2.push(["　" + it.name, n2(ib), n2(ia), n2(ia - ib), ib || ia ? rate(ib, ia) : "—", ""]);
      });
    });
    const TB = allB(), TA = allA();
    s2.push([]);
    s2.push(["合计", n2(TB), n2(TA), n2(TA - TB), rate(TB, TA), ""]);
    const rv = S.meta.reserve || {};
    if (rv.amount) {
      s2.push([`${rv.name || "预留现金"}（预算外）`, n2(rv.amount), n2(rv.used || 0), "", "", rv.note || ""]);
    }
    s2.push([]);
    s2.push(["复核（审批人）：", "", "负责人：", "", "支出预算口径：年度计划值；实际口径：手工补录 + 审批通过/已打款的提报", ""]);
    const ws2 = X.utils.aoa_to_sheet(s2);
    ws2["!cols"] = [{ wch: 30 }, { wch: 14 }, { wch: 22 }, { wch: 16 }, { wch: 10 }, { wch: 28 }];
    ws2["!merges"] = [{ s: { r: 0, c: 0 }, e: { r: 0, c: 5 } }];
    const wb = X.utils.book_new();
    X.utils.book_append_sheet(wb, ws1, `${mLabel}收支明细账`);
    X.utils.book_append_sheet(wb, ws2, "年度预算执行汇总");
    X.writeFile(wb, `${year}年度预算执行表_${mLabel}.xlsx`);
    toast("✅ 已导出 Excel（明细账 + 年度汇总两个工作表）");
  } catch (e) {
